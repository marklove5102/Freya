...
typedef
   struct _Trace_Block {
      ...
      HChar*               name;        // Name of a tree node.
   }
   Trace_Block;
...
static void check_address(Addr ips, Trace_Hash* hash_entry)
{
   ...
   while (rule_ptr) {
      ...
      if (rule_ptr->func_name) {
         if (rule_ptr->func_name_len <= fnname_len && fnname_ptr[0] == rule_ptr->func_name[0]
                && VG_(memcmp)( fnname_ptr, rule_ptr->func_name, rule_ptr->func_name_len * sizeof(HChar)) == 0 ) {

            if (!rule_ptr->is_namespace) {
               if (rule_ptr->func_name_len == fnname_len || fnname_ptr[rule_ptr->func_name_len] == ' ' || fnname_ptr[rule_ptr->func_name_len] == '(')
                  --match;
            } else {
               if (fnname_ptr[rule_ptr->func_name_len] == ':' && fnname_ptr[rule_ptr->func_name_len + 1] == ':')
                  --match;
            }
         }
      }
...
static HChar* fr_get_name(Addr ips)
{
   HChar* dst_ptr = dir_buffer;
   UInt linenum, len, dst_len = DIR_BUFFER_SIZE - 1;
   HChar* name_ptr;
   const HChar* fnname_ptr;
   const HChar* file_ptr;
   const HChar* dir_ptr;
...
static Trace_Block* alloc_trace(ThreadId tid)
{
   ...
   do {
      ...
      if (!block) {
         ...
         block->peak = 0;
         block->ips = *ips_ptr;
         block->name = fr_get_name( *ips_ptr );
      }
